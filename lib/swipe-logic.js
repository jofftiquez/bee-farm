...
const { findActionButtons, handleMatchNotification } = require('./ui-detection');
...
async function performSwipe(page, direction, buttonPositions) {
    ...
    try {
        if (direction === 'right') {
            ...
            // Check for match notification after liking
 ...
 * @param {Object} profileInfo Profile information including bio
 * @param {Object} userPreferences User preferences for matching
 * @returns {Object} Object containing decision and reason
 */
async function processProfile(page, profileInfo, userPreferences) {
    try {
        // Find the action buttons dynamically
        logger.log('Detecting button positions...', logger.LOG_LEVELS.INFO, 'UI');
        const buttonResult = await findActionButtons(page);
        const buttonPositions = buttonResult.buttonPositions;
        
        logger.log(`Using button positions - Like: (${buttonPositions.likeButtonX}, ${buttonPositions.likeButtonY}), Pass: (${buttonPositions.passButtonX}, ${buttonPositions.passButtonY})`, 
                  logger.LOG_LEVELS.INFO, 'UI');
        
        // Extract isVerified from profileInfo
        const isVerified = profileInfo.isVerified || false;
        ...
        if (!userPreferences || typeof userPreferences !== 'object') {
            ...
            logger.log('Using fallback user preferences with LLM enabled', logger.LOG_LEVELS.WARNING, 'CONFIG');
        }
        ...
        // Return the decision and reason for the UI
        return {
            decision: swipeDirection === 'right' ? 'like' : 'dislike',
            reason: swipeDirection === 'right' ? 
                'Profile matches your preferences' : 
                'Profile does not meet your criteria'
        };
    } catch (error) {
