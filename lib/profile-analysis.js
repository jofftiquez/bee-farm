 ...
 */
function checkAvoidKeywords(bio, avoidKeywords) {
    ...
    for (const keyword of avoidKeywords) {
        ...
        if (bioLower.includes(keywordLower)) {
...
async function analyzeProfileAlignment(profileInfo, userPreferences) {
    ...
    // This is a placeholder for the LLM integration with Llama
    ...
    // Count matching keywords in profile bio
    const bioLower = profileInfo.bio.toLowerCase();
    let matchCount = 0;
    ...
    // Combined score with more weight to explicit interests
    const alignmentScore = (keywordAlignmentScore * 0.4) + (interestAlignmentScore * 0.6);
    
    // Decision threshold: adjust as needed
    const alignmentThreshold = userPreferences.alignmentThreshold || 0.3;
    const isAligned = alignmentScore >= alignmentThreshold;
    
    const result = {
        alignmentScore,
        alignmentLevel: alignmentScore < 0.2 ? 'Low' : 
                        alignmentScore < 0.5 ? 'Medium' : 'High',
        isAligned,
        matchingInterests,
        matchCount,
