...
async function extractProfileInfo(page) {
    ...
    try {
        ...
        const profileInfo = await page.evaluate(() => {
            ...
            let bioText = bioElement ? bioElement.textContent.trim() : '';
            
            // Sanitize the bio text to remove any script or tracking code
            if (bioText) {
                // Remove potential script tags or tracking elements
                const scriptPattern = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi;
                const iframePattern = /<iframe\b[^<]*(?:(?!<\/iframe>)<[^<]*)*<\/iframe>/gi;
                const trackingPattern = /window\..*tracking|window\.Bumble|__tcfapi|\$vars|googletagmanager/gi;
                
                // Replace those patterns with empty string
                bioText = bioText
                    .replace(scriptPattern, '')
                    .replace(iframePattern, '')
                    .replace(trackingPattern, '')
                    .replace(/\s+/g, ' ') // Replace multiple spaces with a single space
                    .trim();
            }
            
            // If the bioText is too short or contains suspicious content, try to get full page text
            if (!bioText || bioText.length < 10 || bioText.includes('window.Bumble') || bioText.includes('googletagmanager')) {
                // Fall back to getting all visible text from the profile page
                bioText = getPageTextExcludingSidebar() || '';
                
                // Further clean this text
                if (bioText) {
                    // Remove script-like content and other noise
                    bioText = bioText
                        .replace(/window\.Bumble\s*=.*?;/g, '')
                        .replace(/\$vars\s*=.*?;/g, '')
                        .replace(/__tcfapi\(.*?\);/g, '')
            ...
            // Final cleanup of bio text to ensure it's presentable
            function cleanupBioText(text) {
                if (!text) return '';
                
                // Remove known tracking and script patterns
                return text
                    ...
                    .replace(/\$vars\s*=.*?;/g, '')
                    .replace(/__tcfapi\(.*?\);/g, '')
                    .replace(/GoogleTagManager|GTM-[A-Z0-9]+/g, '')
                    .replace(/taboola/g, '')
                    // Remove common UI text that might be captured
                    .replace(/Block and report|Pass|SuperSwipe|Like|Scroll up|Scroll down/g, '')
                    // Clean up whitespace
                    .replace(/\s+/g, ' ')
                    .trim();
            }
            
            // Clean up the bio and page text
            const cleanBio = cleanupBioText(bioText);
            const cleanPageText = cleanupBioText(pageText);
            
            // Extract the most meaningful bio text
            // If bio is too short or suspicious, try to extract meaningful content from page text
            let finalBioText = cleanBio;
            if (!finalBioText || finalBioText.length < 20 || finalBioText.includes('window.') || finalBioText.includes('tracking')) {
                // Try to extract a meaningful section from the page text
                // Look for keywords that might indicate the start of a bio section
                const bioStartPatterns = [
                    'About', 'Bio', 'Description', 'Interests', 'Hobbies',
                    'I am', 'I\'m', 'My name is', 'I like', 'I love'
                ];
                
                for (const pattern of bioStartPatterns) {
                    const index = cleanPageText.indexOf(pattern);
                    if (index !== -1) {
                        // Extract text after this pattern, up to a reasonable length
                        const bioCandidate = cleanPageText.substring(index, index + 300);
                        if (bioCandidate.length > 30 && !bioCandidate.includes('window.')) {
                            finalBioText = bioCandidate;
                            break;
                        }
                    }
                }
                
                // If we still don't have good bio text, look for personal attributes in page text
                if (!finalBioText || finalBioText.length < 20) {
                    // Look for age mentions, job titles, etc.
                    const personalInfoPatterns = [
                        /\d+ years old/i, /I work as/i, /I live in/i,
                        /graduated from/i, /studied at/i
                    ];
                    
                    for (const pattern of personalInfoPatterns) {
                        const match = cleanPageText.match(pattern);
                        if (match && match.index) {
                            // Extract text around this match
                            const start = Math.max(0, match.index - 50);
                            const bioCandidate = cleanPageText.substring(start, match.index + 250);
                            if (bioCandidate.length > 30 && !bioCandidate.includes('window.')) {
                                finalBioText = bioCandidate;
                                break;
                            }
            ...
            return {
                ...
                age: age || '',
                isVerified: isVerified,
                bio: finalBioText || cleanBio || 'No bio available',
                hasBio: Boolean(finalBioText || cleanBio),
                attributes: attributes,
                hasAttributes: attributes.length > 0,
                location: locationText,
                distance: distanceText,
...
async function analyzeProfileAlignment(profileInfo, userPreferences) {
    ...
    if (userPreferences.llmSettings && userPreferences.llmSettings.enabled) {
        logger.log(`LLM analysis enabled (${JSON.stringify(userPreferences.llmSettings)})`, logger.LOG_LEVELS.INFO, 'LLM');
        
        try {
            logger.log('Checking connection to Llama 3...', logger.LOG_LEVELS.INFO, 'LLM');
