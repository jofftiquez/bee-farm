...
async function saveSession(page) {
  try {
    ...
    const criticalCookies = cookies.filter(c => 
      ['_hive_id', 'user_favorite', 'normal_login'].includes(c.name)
    );
    
    // Combine all important cookies
    const importantCookies = [...combinedCookies, ...criticalCookies];
    ...
    // Save filtered cookies to main file
